...
use guyu_mate_lib::{AppError, AppType, ConfigService, MultiAppConfig, Provider};
...
fn sync_enabled_to_codex_writes_enabled_servers() {
    ...
    let path = guyu_mate_lib::get_codex_config_path();
    ...
    guyu_mate_lib::sync_enabled_to_codex(&config).expect("sync codex");
...
fn sync_enabled_to_codex_returns_error_on_invalid_toml() {
    ...
    let err = guyu_mate_lib::sync_enabled_to_codex(&config).expect_err("sync should fail");
    match err {
        guyu_mate_lib::AppError::Toml { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "path should reference config.toml"
            );
        }
        guyu_mate_lib::AppError::McpValidation(msg) => {
            assert!(
...
"#;
    ...
    guyu_mate_lib::write_codex_live_atomic(&auth, Some(config_text))
        .expect("atomic write should succeed");

    let auth_path = guyu_mate_lib::get_codex_auth_path();
    let config_path = guyu_mate_lib::get_codex_config_path();
    assert!(auth_path.exists(), "auth.json should be created");
    assert!(config_path.exists(), "config.toml should be created");

    let stored_auth: serde_json::Value =
        guyu_mate_lib::read_json_file(&auth_path).expect("read auth");
...
"#;
    ...
    let err = guyu_mate_lib::write_codex_live_atomic(&auth, Some(config_text))
        .expect_err("config write should fail when target is directory");
    match err {
        guyu_mate_lib::AppError::Io { path, .. } => {
            assert!(
                path.ends_with("config.toml"),
                "io error path should point to config.toml"
            );
        }
        guyu_mate_lib::AppError::IoContext { context, .. } => {
            assert!(
...
"#,
    ...
    let mut config = MultiAppConfig::default();
    let changed = guyu_mate_lib::import_from_codex(&mut config).expect("import codex");
...
fn sync_claude_enabled_mcp_projects_to_user_config() {
    ...
    guyu_mate_lib::sync_enabled_to_claude(&config).expect("sync Claude MCP");

    let claude_path = guyu_mate_lib::get_claude_mcp_path();
...
fn import_from_claude_merges_into_config() {
    ...
    let changed = guyu_mate_lib::import_from_claude(&mut config).expect("import from claude");
...
fn create_backup_skips_missing_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_path = home.join(".guyu-mate").join("config.json");
...
fn create_backup_generates_snapshot_file() {
    ...
    reset_test_fs();
    let home = ensure_test_home();
    let config_dir = home.join(".guyu-mate");
...
fn export_sql_returns_error_for_invalid_path() {
    ...
    let invalid_parent = if cfg!(windows) {
        std::env::temp_dir().join("guyu-mate-test-invalid<>dir")
    } else {
...
#[test]
fn import_sql_rejects_non_guyu_mate_backup() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
...
#[test]
fn import_sql_accepts_guyu_mate_exported_backup() {
    let _guard = test_mutex().lock().expect("acquire test mutex");
    reset_test_fs();
    let home = ensure_test_home();
    ...
    let state = create_test_state_with_config(&config).expect("create test state");
    let export_path = home.join("guyu-mate-export.sql");
    state
        .db
        .export_sql(&export_path)
        .expect("export should succeed");

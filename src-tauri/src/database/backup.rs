...
const GUYU_MATE_SQL_EXPORT_HEADER: &str = "-- GuYu Mate SQLite 导出";
...
impl Database {
    ...
    pub fn import_sql_string(&self, sql_raw: &str) -> Result<String, AppError> {
        let sql_content = sql_raw.trim_start_matches('\u{feff}');
        Self::validate_guyu_mate_sql_export(sql_content)?;
    ...
    fn validate_guyu_mate_sql_export(sql: &str) -> Result<(), AppError> {
        let trimmed = sql.trim_start();
        if trimmed.starts_with(GUYU_MATE_SQL_EXPORT_HEADER) {
            return Ok(());
        }

        Err(AppError::localized(
            "backup.sql.invalid_format",
            "仅支持导入由 GuYu Mate 导出的 SQL 备份文件。",
            "Only SQL backups exported by GuYu Mate are supported.",
        ))
    }
    ...
    pub(crate) fn backup_database_file(&self) -> Result<Option<PathBuf>, AppError> {
        let db_path = get_app_config_dir().join("guyu-mate.db");
        if !db_path.exists() {
            return Ok(None);
        }
